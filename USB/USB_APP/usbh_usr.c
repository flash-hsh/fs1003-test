...
//	LCD_DisplayString(30,180,210,16,16,tbuf);	
	sprintf((char*)tbuf,"X POS:%05d",x); 
//	LCD_DisplayString(30,200,200,16,16,tbuf);	
	sprintf((char*)tbuf,"Y POS:%05d",y);
	//LCD_DisplayString(30,220,200,16,16,tbuf);	
	sprintf((char*)tbuf,"Z POS:%05d",z);
	//LCD_DisplayString(30,240,200,16,16,tbuf);	 
...
{ 
	static u16 pos;
    static u16 FS_CHANNEL_num=0;   //FS的通道
    static u8  sendbuff[30];
	static u8  bcd[30];
   // static u32  f;
    static u8 i=0,channel=0;
//    static u8 a=1,b=8;
...
//    printf("string = %s integer = %d\n", str, f);  
	...
	if(USB_FIRST_PLUGIN_FLAG)//第一次插入,将数据清零
	{
        ...
        sendbuff[0]=ADDRESS_SCANNING;  //地址
        sendbuff[2]=SCANNING_DATA_LTH;
        
        USB_FIRST_PLUGIN_FLAG=0;
//		endx=((lcd_width-30)/8)*8+30;		//得到endx值
//		endy=((lcd_height-220)/16)*16+220;	//得到endy值
//		maxinputchar=((lcd_width-30)/8);
//		maxinputchar*=(lcd_height-220)/16;	//当前LCD最大可以显示的字符数.
 		pos=0; 
	}
	//BRUSH_COLOR=BLUE;
	sprintf((char*)buf,"%02X",data);
	//LCD_DisplayString(30+56,180,16,buf);//显示键值	 
	if(data>=' '&&(data<='~')&&(data!=0x20))
	{
		tbuf[pos++]=data;
		tbuf[pos]=0;		//添加结束符. 
		if(pos>12)pos=0;//最大输入这么多
	}
...
//	}
	if(pos==12)	//没有超过显示区
	{
		//LCD_Fill_onecolor(30,220,endx,endy,WHITE);
		//LCD_DisplayString(30,220,16,tbuf);
        //sprintf((char*)trn,"%s",tbuf);
        //sprintf((char*)sendbuff,"%d%d%d%s",ADDRESS_PC,DATA_LTH,FS_CHANNEL_num,tbuf);
       //f=atoi((const char *)tbuf); 

		ASC2BCD(bcd,tbuf,12);  //asciii转BCD码
		for(i=0;i<6;i++)
		{		
			sendbuff[3+i]=bcd[i];		
		}
        if(sendbuff[3]==0x18 && sendbuff[4]==0x51 && sendbuff[8]==0x28)
        {
	    ...
	    sendbuff[10] = (u8)(CRC16(sendbuff, 9));        
          ...
          {		 			  
			  ...
			  idReceive2ScanningBuff[i] = sendbuff[i];			  
          }
       if(FS_CHANNEL_num==4) FS_CHANNEL_num=0;
