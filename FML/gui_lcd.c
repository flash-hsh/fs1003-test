...
//修改记录：无
//******************************************************************
void TFT_GUI_DrawPoint(u16 x,u16 y,u16 color)
{
...
{
	LCD_Fill(x1,y1,x2,y2,PointColor);
...
{  
	...
	if(!mode) //非叠加方式
	{
		...
		for(pos=0;pos<size;pos++)
		{
			...
			else if(size==24) temp=asc2_2412[num][pos];	//调2412字体
			else 			  temp=asc2_3216[num][pos];	//调3216字体
			for(t=0;t<size/2;t++)
		    {                 
...
//修改记录：无
//******************************************************************  	  
void LCD_ShowString(u16 x,u16 y,u16 pc,u16 bc,u8 *p,u8 size,u8 mode)
{         
    while((*p<='~')&&(*p>=' '))//判断是不是非法字符!
    {   
		if(x>(lcddev.width-1)||y>(lcddev.height-1)) 
		return;     
        LCD_ShowChar(x,y,pc,bc,*p,size,mode);
        x+=size/2;
        p++;
    }  
} 
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar(x+(size/2)*t,y,PointColor,BackColor,' ',size,0);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar(x+(size/2)*t,y,PointColor,BackColor,temp+'0',size,0); 
	}
} 
        ...
        if(!bHz)
        {
	        ...
	        {          
		        ...
		        else
				{
					...
					{  
						LCD_ShowChar(x,y,fc,bc,*str,32,mode);
						x+=16; //字符,为全字的一半 
					}
					else if(size==24)//字库中没有集成12X24 16X32的英文字体,用8X16代替
					{  
