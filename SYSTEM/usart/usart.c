...
void FS_uart_init(u32 bound)
{
   ...
   //GPIO端口设置
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART2|RCC_APB1Periph_USART3|RCC_APB1Periph_UART4,ENABLE);//使能USART1时钟
...
void USART_SendChars(USART_TypeDef* USARTx,u8 *str)
{
	u16 k = 0;
	u16 strlen = 0;
	while(*str != '\0')
	...
	do { USART_SendChar(USARTx,*(str + k)); k++; }     
	while (k < strlen); 
}
